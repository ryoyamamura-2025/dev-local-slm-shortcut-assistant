...
| M365 Copilotへ送信 | `copilot_chat(message, model)` | `act Copilotに「この文章を要約して」と送って` |
...
DiskPartで圧縮します。DiskPartの起動時にはWindowsの管理者権限確認が表示されます。
...
`copilot_chat()`はM365 Copilot（`https://m365.cloud.microsoft/chat`）を
Edgeで開き、UI Automationでチャット欄へメッセージを貼り付けて送信します。
`model`は`Opus`、`Think Deeper`、`Quick Response`、`自動`を想定し、
指定がなければ`Opus`を使います。クリップボードの内容をCopilotへ送る依頼では、
`get_clipboard_text → copilot_chat`のチェーンとして実行され、実データはSLMへ渡しません。
...
| Microsoft Edge | M365 Copilotの表示とチャット送信に使用 |
...
22. M365 Copilotへメッセージを送信する操作を追加
...
act クリップボードの内容といっしょに「田中さんに返信」ってタスクを登録
act Copilotに「この文章を要約して」と送って
act クリップボードの内容をCopilotにOpusで送って
...
- M365 CopilotはEdge上の許可済みURLを開き、固定したUI Automation処理で送信する
- ゴミ箱を空にする操作は確認を拒否すると実行されない
- 確認を受け入れた場合だけ固定した`Clear-RecycleBin`処理へ進む
...
- Copilot送信はM365 Copilotへサインイン済みで、EdgeのUI名が現在の想定と一致する必要がある
- Copilot送信は画面のフォーカスや表示タイミングの影響を受けることがある
...
また、ツール名はPython側で登録した一覧に限定します。UI AutomationにはPython側へ固定したPowerShellスクリプトだけを使用し、モデルが任意のPowerShellコマンドやPythonコードを生成・実行する設計にはしません。Copilot送信でも、モデルが決めるのは送信本文とモデル名までで、URL起動、モデル選択、貼り付け、送信は固定実装だけで行います。
