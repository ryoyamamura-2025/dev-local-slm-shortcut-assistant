...
クリップボードの内容をメモする依頼では、`get_clipboard_text()`の戻り値を
`create_text_note()`の`text`へPythonの実行器が渡します。カレンダーの本文（`body`）
も同様に、クリップボードなど直前の操作結果を実行器が注入できます。依頼文に本文が
示されていればSLMが`body`へ入れ、両方が揃った場合は「モデルが入れた本文」+改行+
`===`+改行+「注入された本文」の順に結合し、ICSの`DESCRIPTION`へ入れます。
クリップボードの実データはSLMへ渡しません。

```powershell
act クリップボードの内容をメモとして保存
```
...
Ollama呼び出しでは次を指定し、Actionを選択した際の思考内容をターミナルへ出力します。
...
思考内容は`response.message.thinking`、Structured OutputsのAction計画は
`response.message.content`からそれぞれ取得します。
