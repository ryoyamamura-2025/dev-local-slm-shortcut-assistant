...
| ゴミ箱を開く | `open_recycle_bin()` | `act ゴミ箱の中を見せて` |
| ゴミ箱を空にする | `empty_recycle_bin()` | `act ゴミ箱を空にして` |

`save_current_page()`は直近のChromeまたはEdgeウィンドウからページタイトルとURLを読み取り、プロジェクト直下の`saved_pages.md`へMarkdownリンクとして追記します。このファイルはローカルデータとして`.gitignore`の対象です。
