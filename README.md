...
| カレンダータスクを登録（ダミー） | `create_calendar_task(title, body)` | `act 「田中さんに返信」をカレンダーに登録して` |
...
`create_calendar_task()`は複数Actionの直列実行を確認するためのダミーです。
件名と本文を受け取りますが、現時点ではICSファイルの作成やOutlookへの登録を
行いません。

クリップボードの内容を本文として渡す依頼では、SLMは次の順序付きAction計画だけを
Structured Outputとして返します。クリップボードの実データはSLMへ渡さず、
Pythonの実行器が直前の戻り値を`create_calendar_task()`の`body`へ注入します。
...
Action関数自体はクリップボードへ依存しません。単独で呼ばれた場合は`body`の
既定値である空文字を使用します。複数Actionは最大3個とし、後続Actionが
`accepts_previous_as`で宣言した引数だけへ直前の戻り値を渡します。
...
- カレンダータスク操作はダミーで、ICS作成とOutlook登録は未実装
