...
本文（`body`）は2通りで入ります。依頼文に本文が示されていればSLMが`body`へ入れ、
単独のActionでもそのまま本文付きで登録できます。クリップボードなど直前の操作結果を
本文にする場合は、SLMは`body`を空にし、Pythonの実行器が直前の戻り値を`body`へ
注入します。両方が揃った場合は「モデルが入れた本文」+改行+`===`+改行+「注入された
本文」の順に結合し、ICSの`DESCRIPTION`へ入れます。クリップボードの実データはSLMへ
渡しません。
